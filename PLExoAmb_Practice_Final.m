function myscreen = PLExoAmb_Practice_Final(Observer,Eyetrack,Diag)

% PRACTICE FUNCTION: 2AFC orientation discrimination 4 deg tilt from horizontal w/ 1 target and 1
% distractor using only NEUTRAL PRECUES 

% DIFFERENCES FROM MAIN TRAINING TASK
% 1) Gabor contrast always 64% 
% 2) Stimulus eccentricity is 4 dva
% 3) Stimulus sf is 6 cpd
...
task{1}.segmin =     [1 Inf .01 .06 .06 .12 .7 5 1 10 .2 5 5 .1]; % 10 sec break
...
n_repeats = 40; % n_repeats * stimulus conditions (2) =  80 trials with break halfway for regular training

if Diag == 1 % target can only be in UR or LL quadrant during training for that observer
    [location, repeat] = ndgrid([1,3],1:n_repeats); % two training locations, *n repetitions
else Diag == 2 % target can only be in UL or LR quadrant during training for that observer
    [location, repeat] = ndgrid([2,4],1:n_repeats); % two training locations, *n repetitions
end
...
elseif (task.thistrial.thisseg == 6) % GABOR STIMULI & RESPONSE CUE
     ...
     for Loc=1:length(stimulus.tmp.distractorIndices)
         eval(sprintf('drawGabor(stimulus.contrasts,stimulus.tmp.distractorLocations{Loc}, stimulus.rotation(task.thistrial.distractorOrientation%g), 1);',Loc));
     end
