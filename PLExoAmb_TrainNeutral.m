function myscreen = PLExoAmb_TrainNeutral(Observer,Eyetrack,Contrasts,Diag,Day,Block)
...
% Contrasts: individual's 2 contrast levels [ct dmax] in 1x2 vector
...
stimulus.Tilt=4; % degrees from horizontal
...
n_repeats = 20; % n_repeats * stimulus conditions (4) =  80 trials with break halfway for regular training
...
elseif (task.thistrial.thisseg == 3) % Neutral cue
    mglFixationCross(stimulus.FCwidth,stimulus.FClinewidth,stimulus.black);
...
elseif (task.thistrial.thisseg == 9) % halfway through block (i.e. 40 trials), text will appear onscreen to ask observer to take 1 min break
